...
def fight(participant1, participant2, first2attack):
    ...
    while hp1 > 0 and hp2 > 0:
        if first2attack == 1:
            mult1 = attack_multiplier(participant1[1], participant2[1])
            damage1 = participant1[3] * mult1
            hp2 -= damage1
        if first2attack != 1:
            mult2 = attack_multiplier(participant2[1], participant1[1])
...
def tournament(participants):
    ...
    for i in range(len(wins)):
        for j in range(len(wins)):
            ...
            if away[0] == 1: wins[i] += 1
            if away[0] == 2: wins[j] += 1
