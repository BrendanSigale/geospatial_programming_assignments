...
#     - Clip data into the necessary boundaries.
#         - This is first done by clipping the hwm input to svi, and then doing the reverse using a spatial join on svi.
...
#     - Specified summary statistics are computed for the provided inputs.
...
def evacuationAccessibilityandPSVI(svi, evac, roads, hwm, workspace, joinField, statField, statType, output_path, output_name, fieldmap = None):
    ...
    try:
        ...
        arcpy.analysis.SummarizeWithin(clipped_svi_hwm, hwm_clip, hwm_summary, 'KEEP_ALL', [[statField, statType]])
